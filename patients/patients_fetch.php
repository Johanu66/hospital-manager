<?php
    ...
    $colonne = array("id_personne", "photo_personne", "nom_personne", "email_personne");

    $query = "";

    $output = array();

    $query .= "SELECT * FROM personne INNER JOIN patient ON id_personne = id_personne_fk_patient "; // changer

    if(isset($_POST["search"]["value"]))
    {	// changer les colonnes à rechercher
        $query .= "WHERE ( CONCAT(id_personne, '') LIKE '%".$_POST['search']['value']."%' ";
        ...
        $query .= "OR CONCAT(sexe_personne, '') LIKE '%".$_POST['search']['value']."%' ";
        $query .= "OR CONCAT(date_naissance_personne, '') LIKE '%".$_POST['search']['value']."%' ";
    ...
    foreach($result as $row)
    {
        ...
        $sub_array[] = date("d-m-Y", strtotime($row['date_naissance_personne']));
        ...
        <a href="#" class="dropdown-item view" id="'.$row["id_patient"].'">Consulter</a>';
        if($_SESSION['id_type_compte'] <= 3){
            $actions .= "<a class='dropdown-item' href='patients_edit.php?id=".$row['id_patient']."'>Modifier</a>";
        }
        if($_SESSION['id_type_compte'] <= 2){
            $actions .= "<a class='dropdown-item delete' href='#' id='".$row['id_patient']."'>Supprimer</a>";
        }
        if($_SESSION['id_type_compte'] <= 1){
            if($row['statut_personne']=="Actif"){
                $actions .= "<a class='dropdown-item' href='patients_rendre_inactif.php?id=".$row['id_patient']."'>Désactiver</a>";
            }
            else if($row['statut_personne']=="Inactif"){
                $actions .= "<a class='dropdown-item' href='patients_rendre_actif.php?id=".$row['id_patient']."'>Activer</a>";
            }
        }
        $actions .= '</div>
        </div>';
