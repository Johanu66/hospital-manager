                    ...
                    <div class="container-fluid">
                            ...
                            <div class="row align-items-end">
                                <div class="col-lg-8">
                                    <div class="page-header-title">
                                        ...
                                        <div class="d-inline">
                                            <h5>Modification de recette</h5>
                                            <span>En remplissant ce formulaire, vous pouvez modifier une recette</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                                    <nav class="breadcrumb-container" aria-label="breadcrumb">
                                        <ol class="breadcrumb">
                                            ...
                                            <li class="breadcrumb-item active" aria-current="page">Modifier recette</li>
                                        </ol>
                                    </nav>
                                </div>
                            </div>
                        ...
                        <?php
                                ...
                                if(!empty($_POST['id']) && !empty($_POST['date']) && !empty($_POST['departement']) && !empty($_POST['prestation']) && !empty($_POST['montant'])){
                                    ...
                                    if($montant_paye_par_assure_correct){
                                        if((!isset($_POST['nouveau_patient']) && !empty($_POST['patient'])) | (isset($_POST['nouveau_patient']) && !empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['email']) && !empty($_POST['sexe']) && !empty($_POST['adresse']) && !empty($_POST['tel']) && !empty($_POST['date_naissance']))){
                                            ...
                                            ?>
                                            <script>
                                                window.location.replace("recettes.php?success=La recette a été modifiée.");
                                            </script>
                                            <?php
                                        }
                                        else{
                                            ?>
                                                <script>swal("Échoué", "Veuillez renseigner 1 tous les chmaps.", "error")</script>
                                            <?php
                                        }
                                    }
                                    else{
                            ...
                            if(isset($_GET['id']) && !empty($_GET['id'])){
                                $statement = $bdd->prepare("SELECT * FROM recette INNER JOIN prestation ON id_prestation_fk_recette=id_prestation INNER JOIN departement ON id_departement = id_departement_fk_prestation INNER JOIN patient ON id_patient_fk_recette=id_patient INNER JOIN personne ON id_personne_fk_patient=id_personne WHERE del_recette='0' AND id_recette = ?");
                                $statement->execute(array($_GET['id']));
                                $old_value = $statement->fetch();
                                $_POST['date'] = $old_value['date_recette'];
                                $_POST['departement'] = $old_value['id_departement'];
                                $_POST['prestation'] = $old_value['id_prestation'];
                                $_POST['montant'] = $old_value['montant_prestation'];
                                $_POST['patient'] = $old_value['id_patient'];
                                if($old_value['patient_assure_recette']=="Oui"){
                                    $_POST['patient_assure'] = $old_value['patient_assure_recette'];
                                }
                                $_POST['assureur'] = $old_value['assureur_recette'];
                                $_POST['montant_paye_par_assureur'] = $old_value['montant_paye_par_assureur_recette'];
                                $_POST['notes_recette'] = $old_value['notes_recette'];
                            }
                        ?>
                        <div class="row">
                            <div class="m-auto col-md-11">
                                    <div class="card">
                                        <div class="card-header"><h3>Formulaire de modification de recette</h3></div>
                                        <div class="card-body">
